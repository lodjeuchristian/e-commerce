CREATE OR REPLACE PROCEDURE livrespartheme(numtheme number, theorder varchar2, thedirection varchar2)
IS
...
HTP.print('		
	...
	<div class="container conteneurwtborder" id="fadeOutIn"> 
		...
		<div>
			');		
						--Affichage du nom du thème sélectionné
						ligne_theme:= pa_theme.getByNum(numtheme); 
						HTP.print('<div class="pull-left"><h4>Thème : ' || ligne_theme.lib_theme || '</h4></div>');    
					   
 HTP.print('	 
			
			<div class="pull-right">
				<form method="GET" action = "/g07_epg_dad/livrespartheme" class="form-inline">
					<input type="hidden" name="numtheme" value="' || numtheme || '">
					<select name="theorder">
						<option value="titre_livre" selected>Titre</option>
						<option value="prix_livre">Prix</option>
						<option value="langue">Langue</option>
						<option value="date_parution">Date de parution</option> 
					</select>
					<select name="thedirection">
		...
		<hr/>  
			...
			');		
						--Affichage de la liste des livres du thème
						clivres:= pa_livre.getByThemeNumber(numtheme, theorder, thedirection);
						LOOP
...
<div id="myModal" class="modal hide fade" tabindex="-1" >
  <form method="POST" name="ajouterAuPanierForm" action="/g07_epg_dad/ajouteraupanier">   
    ...
    <div class="modal-footer">
	  ...
	  <button type="submit" name="redirecturl" value="livrespartheme?numtheme=' || numtheme || '&' || 'theorder=titre_livre' || '&' || 'thedirection=ASC" class="btn btn-info"> <i class="icon-shopping-cart icon-white"></i> Ajouter</button>
